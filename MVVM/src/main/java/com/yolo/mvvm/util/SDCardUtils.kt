...
object SDCardUtils {
    ...
    fun byte2FitMemorySize(byteNum: Long)= when {
        ...
        byteNum < KB -> String.format("%.3fB", byteNum + 0.0005)
        byteNum < MB -> String.format("%.3fKB", byteNum / KB)
        byteNum < GB -> String.format("%.3fMB", byteNum / MB)
        else -> String.format("%.3fGB", byteNum / GB)
    }
